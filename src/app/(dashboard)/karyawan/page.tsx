...
import { Search, ChevronLeft, ChevronRight, Users, Plus, Pencil, Trash2, Briefcase, UserCheck, UserX } from 'lucide-react';
...
const JABATAN_OPTIONS = ['Guru', 'Staff', 'Kepala Sekolah', 'Wakil Kepala Sekolah', 'Admin', 'Kebersihan', 'Satpam'];
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select className="w-full h-10 rounded-lg border border-gray-300 px-3 text-sm" value={form.jabatan} onChange={(e) => setForm({ ...form, jabatan: e.target.value })} required>
        ...
        <div>
          ...
          <select className="w-full h-10 rounded-lg border border-gray-300 px-3 text-sm" value={form.jenisKelamin} onChange={(e) => setForm({ ...form, jenisKelamin: e.target.value })}>
            <option value="">-</option>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-sm font-medium text-gray-700 mb-1 block">Tanggal Masuk *</label>
          <Input type="date" value={form.tanggalMasuk} onChange={(e) => setForm({ ...form, tanggalMasuk: e.target.value })} required />
        </div>
        <div>
          <label className="text-sm font-medium text-gray-700 mb-1 block">Gaji Pokok</label>
          <Input type="number" value={form.gajiPokok} onChange={(e) => setForm({ ...form, gajiPokok: e.target.value })} placeholder="0" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-sm font-medium text-gray-700 mb-1 block">No. Telp</label>
          <Input value={form.noTelp} onChange={(e) => setForm({ ...form, noTelp: e.target.value })} />
        </div>
        <div>
          <label className="text-sm font-medium text-gray-700 mb-1 block">Status</label>
          <select className="w-full h-10 rounded-lg border border-gray-300 px-3 text-sm" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
      ...
      <div className="flex justify-end gap-2 pt-2">
        <Button type="submit" className="bg-[#059DEA] hover:bg-[#0480c4] text-white">{submitLabel}</Button>
      </div>
    </form>
  );
    ...
    <div className="space-y-6">
      ...
      <Dialog.Root open={isEditOpen} onOpenChange={setIsEditOpen}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 z-50" />
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg z-50 max-h-[90vh] overflow-y-auto">
            <Dialog.Title className="text-lg font-semibold mb-4">Edit Karyawan</Dialog.Title>
      ...
      <Dialog.Root open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm z-50">
            <Dialog.Title className="text-lg font-semibold mb-2">Hapus Karyawan</Dialog.Title>
            <p className="text-sm text-gray-600 mb-4">Yakin ingin menghapus <strong>{selectedEmployee?.nama}</strong>? Data tidak dapat dikembalikan.</p>
