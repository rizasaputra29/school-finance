...
export default function PaymentPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'Belum Lunas' | 'Lunas'>('Belum Lunas');
  ...
  }, [pagination.page, pagination.limit, statusFilter, overdueFilter, debouncedSearchTerm]);
  ...
  const getStatusBadge = (billing: Billing) => {
    ...
    if (billing.isOverdue) {
      return <Badge className="bg-red-100 text-red-800"><AlertCircle className="w-3 h-3 mr-1" />Jatuh Tempo</Badge>;
    }
    return <Badge className="bg-yellow-100 text-yellow-800"><Clock className="w-3 h-3 mr-1" />Belum Lunas</Badge>;
  };
      ...
      <div className="container mx-auto p-4">
        <Card>
          <CardContent className="pt-6">
            <p className="text-center text-gray-500">Akses terbatas untuk admin</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <>
      <Head>
        <title>Pembayaran Siswa - School Finance</title>
      </Head>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <Card>
            <CardContent className="pt-4">
              <div className="text-sm text-gray-500">Total Tagihan Belum Lunas</div>
              <div className="text-2xl font-bold">{summary.totalUnpaid}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-4">
              <div className="text-sm text-gray-500">Total Jatuh Tempo (Piutang)</div>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-8 text-gray-500">Tidak ada tagihan</div>
            ) : (
              <>
                    ...
                    <TableBody>
                      {billings.map((billing) => (
                        <TableRow key={billing.id}>
                          <TableCell>
                            <div>
                              <div className="font-medium">{billing.student.nama}</div>
                              <div className="text-sm text-gray-500">NIS: {billing.student.nis}</div>
                              <div className="text-xs text-gray-400">{billing.student.kelas}</div>
                            </div>
                          </TableCell>
                          <TableCell>{billing.jenisBiaya}</TableCell>
                          <TableCell>
                            {billing.periodeBulan 
                              ? new Date(billing.periodeBulan + '-01').toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })
                              : '-'}
                          </TableCell>
                          <TableCell className="text-right">{formatCurrency(billing.jumlah)}</TableCell>
              ...
              {selectedBilling && (
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    ...
                    <div className="font-medium">{selectedBilling.jenisBiaya}</div>
                    <div className="text-sm">
                      {selectedBilling.student.nama} - {selectedBilling.student.nis}
                    </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setPaymentForm(prev => ({ ...prev, jumlahBayar: value ? formatCurrency(parseInt(value)) : '' }));
                      }}
                      placeholder="0"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setPaymentForm(prev => ({ ...prev, tanggalBayar: e.target.value }))}
                    />
                  </div>
