...
export default function ApprovePage() {
  ...
  return (
    <>
      <Head>
        <title>Persetujuan Transaksi - Keuangan Sekolah</title>
      </Head>

      <div className="space-y-6">
        <div className="flex items-center gap-2">
          <Clock className="h-6 w-6 text-gray-700" />
          <h1 className="text-xl md:text-2xl font-bold text-gray-900">Persetujuan Transaksi</h1>
        </div>
          ...
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-blue-600 font-medium">Menunggu Persetujuan</p>
                  <p className="text-2xl font-bold text-blue-900">{cashflows.length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader className="border-b">
            <CardTitle className="text-lg">Daftar Transaksi Menunggu Persetujuan</CardTitle>
          </CardHeader>
                  ...
                  {cashflows.map((cf) => (
                    <TableRow key={cf.id} className="hover:bg-slate-50/50">
                      ...
                      <TableCell className="text-gray-600">
                        ...
                        <span className="text-gray-400 ml-1">- {accounts[cf.kodeAkun] || ''}</span>
                      </TableCell>
            ...
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="font-medium">{formatShortDate(selectedCashflow.tanggal)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium font-mono">
                    {selectedCashflow.kodeAkun} - {accounts[selectedCashflow.kodeAkun] || ''}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium text-emerald-600">
                      {selectedCashflow.debit > 0 ? formatCurrency(selectedCashflow.debit) : '-'}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500">Kredit</p>
                    <p className="font-medium text-red-600">
                      {selectedCashflow.kredit > 0 ? formatCurrency(selectedCashflow.kredit) : '-'}
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  <Badge variant="secondary" className="mt-1">
                    {selectedCashflow.status === 'draft' ? 'Menunggu' : selectedCashflow.status}
                  </Badge>
                </div>
              </div>
