...
export default function BillingPage() {
  ...
  return (
    <>
      <Head>
        <title>Biaya Siswa - Keuangan Sekolah</title>
      </Head>
                  ...
                  <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                              ...
                              .map((s) => (
                                <button
                                  ...
                                  className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-100 transition-colors ${
                                    formData.studentId === s.id ? 'bg-[#059DEA]/30 font-medium' : ''
                                  }`}
                                >
                                  <span className="font-medium">{s.nis}</span> - {s.nama} <span className="text-gray-500">({s.kelas})</span>
                                </button>
                              ))}
                            {students.filter((s) => 
                              s.nama.toLowerCase().includes(studentSearch.toLowerCase()) ||
                              ...
                              <p className="px-3 py-2 text-sm text-gray-500">Tidak ada siswa ditemukan</p>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, periodeBulan: e.target.value })
                        }
                        required
                      />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-slate-400">Contoh: 500.000 = lima ratus ribu rupiah</p>
                    </div>
          ...
          <Card className="bg-[#059DEA] shadow-sm">
            <CardContent className="flex items-center gap-3 p-3 md:p-5">
              ...
              <div className="min-w-0">
                <p className="text-[10px] md:text-xs font-medium text-white/80 truncate">Total Tagihan</p>
              ...
              <Table className="min-w-[800px]">
                ...
                <TableBody>
                  {filteredBillings.map((b) => (
                    <TableRow key={b.id}>
                      <TableCell className="font-mono">{b.student.nis}</TableCell>
                      <TableCell className="font-medium">{b.student.nama}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Badge variant={b.statusBayar === 'Lunas' ? 'success' : 'warning'}>
                          {b.statusBayar}
                        </Badge>
                      </TableCell>
                      <TableCell>
            ...
            {selectedBilling && (
              <div className="mt-4 space-y-3">
                <div className="rounded-lg bg-slate-50 p-4">
                  <p className="text-sm text-slate-600">
                    <span className="font-medium">Siswa:</span> {selectedBilling.student.nama}
                  </p>
                  <p className="text-sm text-slate-600">
                    <span className="font-medium">Jenis:</span> {selectedBilling.jenisBiaya}
                  </p>
                  <p className="text-sm text-slate-600">
                    <span className="font-medium">Periode:</span> {selectedBilling.periodeBulan}
                  </p>
                ...
                <p className="text-sm text-slate-500">
                  Pembayaran akan otomatis tercatat di Cashflow sebagai pemasukan.
                </p>
              </div>
            )}
