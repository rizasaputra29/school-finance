...
function getPeriodString(periodeBulan?: string | string[]): string {
  if (!periodeBulan) return '';
  const bulan = periodeBulan as string;
  if (bulan.includes('-')) {
    const [start, end] = bulan.split('-');
...
async function handler(
  ...
  try {
    const { format, periodeBulan, statusBayar, studentId, search } = req.query;
    ...
    if (format === 'pdf') {
      buffer = await exportToPDF(billings, summary, periodeBulan as string);
      contentType = 'application/pdf';
      filename = `billing-${new Date().toISOString().split('T')[0]}.pdf`;
    } else {
      buffer = await exportToExcel(billings, summary, periodeBulan as string);
