...
async function handler(
  ...
  try {
    switch (req.method) {
      case 'GET': {
        // GET: Retrieve depreciation status for assets
        const { year: queryYear, assetId } = req.query;
        const year = queryYear
          ? parseInt(queryYear as string, 10)
      ...
      case 'POST': {
        // POST: Trigger depreciation calculation and journal entry generation
        const validationErrors = validateBody(req.body, depreciateSchema);
        if (validationErrors) {
          return res.status(400).json({ errors: validationErrors });
        }

        const { year, assetId, force } = req.body;
...
// Helper function for body validation (replicated from lib/validation)
