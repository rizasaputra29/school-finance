...
function parseQueryParams(query: Record<string, unknown>): QueryParams {
  ...
  return {
    startDate,
    endDate,
    status: query.status && typeof query.status === 'string' ? query.status : null,
    search: query.search && typeof query.search === 'string' ? query.search : null,
    page,
    limit,
  };
}
...
async function handler(
  ...
  try {
    ...
    console.error('Jurnal Report API error:', error);
