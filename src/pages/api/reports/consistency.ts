...
async function calculateLedgerBalances(
  ...
  const balances = new Map<string, { debit: number; kredit: number; saldo: number }>();

  for (const entry of entries) {
    const existing = balances.get(entry.kodeAkun) || { debit: 0, kredit: 0, saldo: 0 };
...
async function runConsistencyCheck(periode?: string): Promise<ConsistencyCheckResult> {
  ...
  const neracaBalance = roundAmount(
    reportTotals.totalAset - (reportTotals.totalKewajiban + reportTotals.totalEkuitas)
  );
  ...
  checks.push({
    ...
    details: isAmountEqual(neracaBalance, 0)
      ? `Aset: ${reportTotals.totalAset.toLocaleString('id-ID')} = Kewajiban + Ekuitas: ${(reportTotals.totalKewajiban + reportTotals.totalEkuitas).toLocaleString('id-ID')}`
      : `Selisih: ${neracaBalance.toLocaleString('id-ID')}`,
    difference: neracaBalance,
  });

  // Check 4: Laba/Rugi consistency
  const labarugi = roundAmount(reportTotals.totalPendapatan - reportTotals.totalBeban);
  checks.push({
    name: 'LABARUGI_CALCULATION',
    description: 'Laba/Rugi harus terhitung dengan benar (Pendapatan - Beban)',
    passed: true,
    details: `Pendapatan: ${reportTotals.totalPendapatan.toLocaleString('id-ID')} - Beban: ${reportTotals.totalBeban.toLocaleString('id-ID')} = ${labarugi.toLocaleString('id-ID')} (${labarugi >= 0 ? 'LABA' : 'RUGI'})`,
    difference: labarugi,
  });
  ...
  checks.push({
    ...
    details: cashflowCheck.passed
      ? `Kas: ${cashflowCheck.cashBalance.toLocaleString('id-ID')} = Arus Kas: ${cashflowCheck.cashflowBalance.toLocaleString('id-ID')}`
    ...
    switch (req.method) {
      case 'GET': {
        ...
        if (tipe && typeof tipe === 'string') {
          const result = await runConsistencyCheck(periode as string | undefined);
        ...
        // Run all checks
        const fullResult = await runConsistencyCheck(periode as string | undefined);
      ...
      case 'POST': {
        ...
        const { periode, check } = req.body;

        if (!check) {
          return res.status(400).json({ error: 'Parameter check wajib diisi' });
        }
