...
async function handler(
  ...
  try {
    switch (req.method) {
      case 'GET': {
        // Get list of piutang (overdue installments)
        const { studentId, aging, minimalHari } = req.query;
        ...
        if (minimalHari) {
          const minDays = parseInt(minimalHari as string, 10);
      ...
      case 'POST': {
        ...
        const validationErrors = validateBody(req.body, payPiutangSchema);
        if (validationErrors) {
          return sendValidationError(res, validationErrors);
        }

        const { installmentId, jumlahBayar, tanggalBayar } = req.body as z.infer<typeof payPiutangSchema>;
