...
export default function KeuanganPage() {
  ...
  const accountsByType = accounts.reduce<Record<string, Account[]>>((acc, a) => {
  ...
  return (
    <>
      <Head><title>Keuangan - School Finance</title></Head>
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Keuangan</h1>
          <p className="text-sm text-gray-500 mt-1">Master data keuangan & manajemen Kas/Bank</p>
        </div>
        ...
        <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
          <button onClick={() => setTab('kas-bank')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${tab === 'kas-bank' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-600 hover:text-gray-900'}`}>
            <span className="flex items-center gap-2"><Wallet className="h-4 w-4" /> Kas & Bank</span>
          </button>
          <button onClick={() => setTab('akun')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${tab === 'akun' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-600 hover:text-gray-900'}`}>
            <span className="flex items-center gap-2"><BookOpen className="h-4 w-4" /> Chart of Accounts</span>
          </button>
        </div>
        ...
        {tab === 'kas-bank' && (
          <>
            ...
            {/* Transfer Button */}
            {isAdmin && (
              <Dialog.Root open={isTransferOpen} onOpenChange={(o) => { setIsTransferOpen(o); if (o) { setError(''); setSuccess(''); } }}>
                ...
                <Dialog.Portal>
                  ...
                  <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-6 w-full max-w-md z-50">
                    <Dialog.Title className="text-lg font-semibold mb-4">Transfer Kas ↔ Bank</Dialog.Title>
                    <form onSubmit={handleTransfer} className="space-y-4">
                      {error && <div className="text-sm text-red-600 bg-red-50 p-3 rounded-lg">{error}</div>}
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-sm font-medium text-gray-700 mb-1 block">Dari</label>
                          <select className="w-full h-10 rounded-lg border border-gray-300 px-3 text-sm" value={transferForm.dari} onChange={(e) => setTransferForm({ ...transferForm, dari: e.target.value, ke: e.target.value === '101' ? '102' : '101' })}>
                            <option value="101">Kas</option>
                            <option value="102">Bank</option>
                          </select>
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-700 mb-1 block">Ke</label>
                          <select className="w-full h-10 rounded-lg border border-gray-300 px-3 text-sm" value={transferForm.ke} onChange={(e) => setTransferForm({ ...transferForm, ke: e.target.value, dari: e.target.value === '101' ? '102' : '101' })}>
                            <option value="101">Kas</option>
                            <option value="102">Bank</option>
                          </select>
                        </div>
                      ...
                      <div>
                        ...
                        <Input type="date" value={transferForm.tanggal} onChange={(e) => setTransferForm({ ...transferForm, tanggal: e.target.value })} required />
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-700 mb-1 block">Jumlah (Rp) *</label>
                        <Input type="number" value={transferForm.jumlah} onChange={(e) => setTransferForm({ ...transferForm, jumlah: e.target.value })} required min="1" placeholder="0" />
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-700 mb-1 block">Keterangan</label>
                        <Input value={transferForm.keterangan} onChange={(e) => setTransferForm({ ...transferForm, keterangan: e.target.value })} placeholder="Opsional" />
                      </div>
                      <div className="flex justify-end gap-2 pt-2">
                        <Button type="submit" className="bg-[#059DEA] hover:bg-[#0480c4] text-white">Transfer</Button>
                      </div>
                    </form>
                  </Dialog.Content>
                </Dialog.Portal>
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <CardTitle className="text-base font-semibold">Riwayat Transfer</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {loading ? (
                        <TableRow><TableCell colSpan={3} className="text-center py-8 text-gray-500">Memuat...</TableCell></TableRow>
                      ) : mutasiHistory.length === 0 ? (
                        <TableRow><TableCell colSpan={3} className="text-center py-8 text-gray-500">Belum ada riwayat transfer</TableCell></TableRow>
                      ) : (
                        mutasiHistory.map((m) => {
                          ...
                          return (
                            <TableRow key={m.id} className="hover:bg-gray-50">
                              <TableCell className="text-sm">{new Date(m.tanggal).toLocaleDateString('id-ID')}</TableCell>
                              <TableCell>
        ...
        {tab === 'akun' && (
          <>
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-500">Daftar akun keuangan (Chart of Accounts)</p>
              <Link href="/accounts">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {(accountsByType[type] || []).map(acc => (
                        <TableRow key={acc.id}>
                          <TableCell className="font-mono text-sm">{acc.kodeAkun}</TableCell>
                          <TableCell>{acc.namaAkun}</TableCell>
                          <TableCell className="text-right font-medium">{formatCurrency(acc.saldo)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
