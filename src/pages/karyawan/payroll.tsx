...
import { ChevronLeft, ChevronRight, Wallet, ArrowLeft, CheckCircle, Clock, Plus } from 'lucide-react';
...
export default function PayrollPage() {
  ...
  const [summary, setSummary] = useState({ totalJumlah: 0, belumBayar: 0, lunas: 0 });
  ...
  return (
    <>
      <Head><title>Gaji & Tunjangan - School Finance</title></Head>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Gaji & Tunjangan</h1>
              <p className="text-sm text-gray-500 mt-1">Pembayaran gaji, tunjangan, dan bonus karyawan</p>
            </div>
          </div>
          {isAdmin && (
            <Dialog.Root open={isCreateOpen} onOpenChange={(o) => {
              setIsCreateOpen(o);
              if (o) { setError(''); setSuccess(''); setForm(f => ({ ...f, employeeId: '', jumlah: '', keterangan: '', periode: periodeFilter })); }
            }}>
              ...
              <Dialog.Portal>
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-6 w-full max-w-md z-50">
                  <Dialog.Title className="text-lg font-semibold mb-4">Pembayaran Gaji/Tunjangan</Dialog.Title>
                  <form onSubmit={handlePayment} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input type="number" value={form.jumlah} onChange={(e) => setForm({ ...form, jumlah: e.target.value })} required min="1" />
                      </div>
                      <div>
                        <label className="text-sm font-medium text-gray-700 mb-1 block">Sumber Dana</label>
                        <select className="w-full h-10 rounded-lg border border-gray-300 px-3 text-sm" value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value as 'kas' | 'bank' })}>
                    ...
                    <div>
                      ...
                      <Input value={form.keterangan} onChange={(e) => setForm({ ...form, keterangan: e.target.value })} placeholder="Opsional" />
                    </div>
                    <div className="flex justify-end gap-2 pt-2">
                      <Button type="submit" className="bg-[#059DEA] hover:bg-[#0480c4] text-white">Bayar</Button>
                    </div>
                  </form>
                </Dialog.Content>
              </Dialog.Portal>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card className="border-0 shadow-sm bg-linear-to-br from-blue-50 to-white">
            <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-600">Total Pengeluaran</CardTitle></CardHeader>
            <CardContent><div className="flex items-center gap-2"><Wallet className="h-5 w-5 text-blue-500" /><span className="text-xl font-bold text-gray-900">{formatCurrency(summary.totalJumlah)}</span></div></CardContent>
          </Card>
          <Card className="border-0 shadow-sm bg-linear-to-br from-green-50 to-white">
            <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-600">Lunas</CardTitle></CardHeader>
            <CardContent><div className="flex items-center gap-2"><CheckCircle className="h-5 w-5 text-green-500" /><span className="text-2xl font-bold text-gray-900">{summary.lunas}</span></div></CardContent>
          </Card>
          <Card className="border-0 shadow-sm bg-linear-to-br from-amber-50 to-white">
            <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-600">Belum Bayar</CardTitle></CardHeader>
            <CardContent><div className="flex items-center gap-2"><Clock className="h-5 w-5 text-amber-500" /><span className="text-2xl font-bold text-gray-900">{summary.belumBayar}</span></div></CardContent>
          </Card>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Input type="month" value={periodeFilter} onChange={(e) => setPeriodeFilter(e.target.value)} className="max-w-[180px]" />
        </div>
              ...
              <Table>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow><TableCell colSpan={6} className="text-center py-8 text-gray-500">Belum ada data pembayaran</TableCell></TableRow>
                  ) : (
                    payrolls.map((p) => (
                      <TableRow key={p.id} className="hover:bg-gray-50">
                        <TableCell>
                          <div>
                            ...
                            <div className="text-xs text-gray-500">{p.employee.nip}</div>
                          </div>
                        </TableCell>
                        <TableCell>{p.employee.jabatan}</TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                            p.jenisPembayaran === 'Gaji' ? 'bg-blue-100 text-blue-700' :
                            p.jenisPembayaran === 'Tunjangan' ? 'bg-purple-100 text-purple-700' :
                            'bg-amber-100 text-amber-700'
                          }`}>{p.jenisPembayaran}</span>
                        </TableCell>
                        <TableCell>{p.periode}</TableCell>
                        <TableCell className="text-right font-medium">{formatCurrency(p.jumlah)}</TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${p.status === 'Lunas' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {p.status}
                          </span>
                        </TableCell>
                      </TableRow>
