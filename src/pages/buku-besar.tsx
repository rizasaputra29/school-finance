...
export default function BukuBesarPage() {
  ...
  return (
    <>
      <Head>
        <title>Buku Besar - Keuangan Sekolah</title>
      </Head>
            ...
            {reports.map((report, index) => (
              <Card key={index}>
                <CardHeader className="text-center border-b pb-4">
                  <CardTitle className="text-xl font-medium">{report.account.kodeAkun} - {report.account.namaAkun}</CardTitle>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-gray-50/50">
                        ...
                        <TableHead className="font-semibold text-gray-700 text-right w-36 border-r">Debet</TableHead>
                        <TableHead className="font-semibold text-gray-700 text-right w-36 border-r">Kredit</TableHead>
                        <TableHead className="font-semibold text-gray-700 text-right w-40 border-r">Saldo</TableHead>
                        <TableHead className="font-semibold text-gray-700 min-w-64">Keterangan</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      <TableRow className="bg-amber-50/10 font-medium">
                        <TableCell colSpan={2} className="border-r">Saldo Sebelumnya</TableCell>
                        <TableCell className="border-r"></TableCell>
                        <TableCell className="border-r"></TableCell>
                        <TableCell className="text-right border-r">{formatCurrency(report.summary.openingBalance).replace('Rp', '')}</TableCell>
                        <TableCell></TableCell>
                      </TableRow>
                      
                      {report.data.map((entry, idx) => (
                        <TableRow key={entry.id || idx} className="hover:bg-slate-50/50">
                          <TableCell className="text-gray-600 border-r py-3">{formatDateTime(entry.tanggal)}</TableCell>
                          <TableCell className="text-gray-800 border-r">{report.account.namaAkun}</TableCell>
                          <TableCell className="text-emerald-700 text-right border-r">{entry.debit > 0 ? entry.debit.toFixed(2) : '0.00'}</TableCell>
                          <TableCell className="text-red-700 text-right border-r">{entry.kredit > 0 ? entry.kredit.toFixed(2) : '0.00'}</TableCell>
                          <TableCell className="font-medium text-right border-r px-2">{entry.saldo.toFixed(0)}</TableCell>
                          <TableCell className="text-gray-600 truncate">{entry.keterangan}</TableCell>
                        </TableRow>
                      ))}
