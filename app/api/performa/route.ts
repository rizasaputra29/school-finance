    ...
    try {
      ...
      const journalLines = await prisma.journalEntryLine.findMany({
        ...
        include: {
          ...
          account: { select: { kodeAkun: true, namaAkun: true, tipeAkun: true } },
        },
      });

      // === Monthly Revenue vs Expense bar chart ===
      const monthlyData: Array<{ bulan: string; pendapatan: number; beban: number; netProfit: number }> = [];
      for (let m = 0; m < 12; m++) {
      ...
      for (const line of journalLines) {
        if (line.account.tipeAkun === 'Revenue' && line.kredit > 0) {
          const name = line.account.namaAkun;
          revenueByCategory[name] = (revenueByCategory[name] || 0) + line.kredit;
        }
      }
      ...
      // === Totals ===
      const totalPendapatan = monthlyData.reduce((sum, m) => sum + m.pendapatan, 0);
      ...
      const initialBank = accounts.find(a => a.kodeAkun === '102')?.saldo || 0;
