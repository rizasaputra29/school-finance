...
export async function GET(request: NextRequest) {
	return withAuthAppRouter(request, async () => {
		...
		if (!bankCode) {
			...
			if (!bankAccount?.kodeAkun) {
				return errors.notFound(
					"Akun Bank tidak ditemukan. Silakan buat akun Bank terlebih dahulu.",
				);
			}
		...
		if (start) {
			...
			const currentDebit = transactions.reduce(
				(sum, t) => sum + (t.debit || 0),
				0,
			);
			const currentKredit = transactions.reduce(
				(sum, t) => sum + (t.kredit || 0),
				0,
			);
		...
		const totalKredit = withdrawals.reduce(
			(sum, t) => sum + (t.kredit || 0),
			0,
		);
