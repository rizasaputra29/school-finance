	...
	return withAuthAppRouter(request, async () => {
		try {
			...
			if (startDate) {
				...
				const priorDebit = priorTransactions.reduce(
					(sum, cf) => sum + cf.debit,
					0,
				);
				const priorKredit = priorTransactions.reduce(
			...
			return success(cashbookEntries, {
				message: "Laporan cashbook berhasil diambil",
				meta: {
					summary: {
