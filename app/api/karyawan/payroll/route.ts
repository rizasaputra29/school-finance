...
const createPayrollSchema = z.object({
	...
	periode: z
		...
		.regex(/^\d{4}-\d{2}$/, "Format periode: YYYY-MM"),
	jenisPembayaran: z.enum(["Gaji", "Tunjangan", "Bonus"], {
		...
		try {
			...
			if (jumlah <= 0) {
				return errors.validation([
					{ field: "jumlah", message: "Jumlah harus lebih dari 0" },
				]);
			}
			...
			const period = await prisma.period.findUnique({
				where: { kode: data.periode },
			});
			if (period && period.status === "closed") {
				return errors.badRequest(`Periode ${data.periode} sudah ditutup`);
			}
			...
			if (existingPayroll) {
				return errors.conflict(
					`${data.jenisPembayaran} untuk ${employee.nama} periode ${data.periode} sudah ada`,
				);
			}
			...
			if (!expenseAccountCode) {
				return errors.validation([
					{ field: "jenisPembayaran", message: "Jenis pembayaran tidak valid" },
				]);
			}
			...
			if (!expenseAccount) {
				return errors.notFound(`Akun beban ${expenseAccountCode}`);
			}
			if (!cashAccount) {
				return errors.notFound(`Akun kas/bank ${cashAccountCode}`);
			}
