...
export async function POST(request: NextRequest) {
  return withAuthAppRouter(request, async (user) => {
    try {
      ...
      if (jumlah <= 0) {
        return errors.validation([{ field: 'jumlah', message: 'Jumlah harus lebih dari 0' }]);
      }
      ...
      if (existingPayroll) {
        return errors.conflict(`${data.jenisPembayaran} untuk ${employee.nama} periode ${data.periode} sudah ada`);
      }

      // Determine accounts
      const expenseAccountCode = PAYMENT_TYPE_ACCOUNTS[data.jenisPembayaran];
      if (!expenseAccountCode) {
        return errors.validation([{ field: 'jenisPembayaran', message: 'Jenis pembayaran tidak valid' }]);
      }
