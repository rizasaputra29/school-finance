  ...
  return withAuthAppRouter(request, async () => {
    try {
      ...
      if (type === 'laba-rugi') {
        ...
        const accounts = await prisma.account.findMany({ where: { tipeAkun: { in: ['Revenue', 'Expense'] } }, orderBy: [{ tipeAkun: 'asc' }, { kodeAkun: 'asc' }] }) as AccountRecord[];
        ...
        autoTable(doc, {
          ...
          body: revenueItems.map((a, i) => [(i + 1).toString(), a.kodeAkun, a.namaAkun, formatCurrency(a.saldo)]),
        ...
        autoTable(doc, {
          startY: finalY1 + 5,
          head: [['No', 'Kode Akun', 'Nama Akun', 'Jumlah (Rp)']],
          body: expenseItems.map((a, i) => [(i + 1).toString(), a.kodeAkun, a.namaAkun, formatCurrency(a.saldo)]),
      ...
      if (type === 'neraca') {
        ...
        autoTable(doc, {
          startY: startY + 5,
          head: [['No', 'Kode Akun', 'Nama Akun', 'Jumlah (Rp)']],
          body: assets.map((a, i) => [(i + 1).toString(), a.kodeAkun, a.namaAkun, formatCurrency(a.saldo)]),
        ...
        autoTable(doc, {
          startY: finalY1 + 5,
          head: [['No', 'Kode Akun', 'Nama Akun', 'Jumlah (Rp)']],
          body: liabilities.map((a, i) => [(i + 1).toString(), a.kodeAkun, a.namaAkun, formatCurrency(a.saldo)]),
        ...
        autoTable(doc, {
          startY: finalY2 + 5,
          head: [['No', 'Kode Akun', 'Nama Akun', 'Jumlah (Rp)']],
          body: equity.map((a, i) => [(i + 1).toString(), a.kodeAkun, a.namaAkun, formatCurrency(a.saldo)]),
