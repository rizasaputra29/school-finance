...
function calculateLabaRugi(
	...
	const totalRevenue = revenueAccounts.reduce(
		(sum, account) =>
			sum + calculateAccountBalance(cashflows, account.kodeAkun, "Revenue"),
		0,
	);
	const totalExpense = expenseAccounts.reduce(
		(sum, account) =>
			sum + calculateAccountBalance(cashflows, account.kodeAkun, "Expense"),
		0,
	);
...
async function exportToPDF(
	...
	const asetData = assetAccounts.map((account) => {
		const jumlah = calculateAccountBalance(
			cashflows,
			account.kodeAkun,
			"Asset",
		);
