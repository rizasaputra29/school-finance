...
export async function POST(request: NextRequest) {
	return withAuthAppRouter(
		...
		async () => {
			...
			const { studentId, jenisBiaya, periodeBulan, jumlah, catatan } =
			...
			try {
				...
				if (existingBilling) {
					return errors.conflict(
						`Tagihan ${jenisBiaya} untuk periode ${periodeBulan} sudah ada untuk siswa ini`,
					);
				}
				...
				const billing = await prisma.billing.create({
					data: {
						...
						periodeBulan: periodeBulan || "",
						jumlah:
							typeof jumlah === "string" ? parseFloat(jumlah) : jumlah || 0,
