  ...
  return withAuthAppRouter(request, async (user) => {
    try {
      ...
      if (overrideClosedPeriod && !isOwner) {
        return errors.forbidden('Hanya owner yang dapat meng-override periode yang sudah ditutup');
      }
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const journal = await tx.journalEntry.update({
          ...
          data: {
            status: newStatus,
            version: { increment: 1 },
            ...(action === 'post' ? { postedAt: new Date(), postedBy: user.id } : {}),
          },
        });

        // Audit trail
        await logAudit(action, 'journal', id, user.id,
