...
export async function getCachedDashboardData() {
	...
	const [totalStudents, totalAccounts, recentCashflows, accountSummary] =
		await Promise.all([
...
export async function getFilteredDashboardCache(
	bulan?: number,
	tahun?: number,
	startDate?: string,
	endDate?: string,
	...
	const [cashflows, accounts] = await Promise.all([
		prisma.cashflow.findMany({
	...
	cashflows.forEach((cf) => {
		if (cf.kredit > 0) {
			...
			expenseByCategory[categoryName] =
				(expenseByCategory[categoryName] || 0) + cf.kredit;
		}
	});
